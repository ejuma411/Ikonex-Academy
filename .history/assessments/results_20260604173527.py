from django.db.models import Sum, Avg
from students.models import Student
from assessments.models import Score
from classes.models import ClassStream


# --------------------------
# STUDENT PERFORMANCE ENGINE
# --------------------------

def student_result(student_id):
    scores = Score.objects.filter(student_id=student_id)

    total = scores.aggregate(total=Sum('marks'))['total'] or 0
    average = scores.aggregate(avg=Avg('marks'))['avg'] or 0

    return {
        "total": total,
        "average": round(average, 2),
        "scores": scores
    }
   
# CLASS RANKING SYSTEM
def class_ranking(class_id):
    students = Student.objects.filter(class_stream_id=class_id)

    ranking = []

    for student in students:
        scores = Score.objects.filter(student=student)

        total = scores.aggregate(total=Sum('marks'))['total'] or 0
        avg = scores.aggregate(avg=Avg('marks'))['avg'] or 0

        ranking.append({
            "student": student,
            "total": total,
            "average": round(avg, 2)
        })

    # sort by performance
    ranking = sorted(ranking, key=lambda x: x["total"], reverse=True)

    # assign positions
    position = 1
    for r in ranking:
        r["position"] = position
        position += 1

    return ranking
 
# GRADE CALCULATION
from assessments.models import GradeScale


def calculate_grade(mark):
    grade = GradeScale.objects.filter(
        min_mark__lte=mark,
        max_mark__gte=mark
    ).first()

    if grade:
        return grade.grade
    return "N/A"
 
# STUDENT RESULT SUMMARY
from django.shortcuts import render, get_object_or_404
from students.models import Student
from .results import student_result, calculate_grade


def student_result_view(request, pk):
    student = get_object_or_404(Student, pk=pk)

    result = student_result(student.id)

    enriched_scores = []

    for s in result["scores"]:
        enriched_scores.append({
            "subject": s.subject.name,
            "assessment": s.assessment.name,
            "marks": s.marks,
            "grade": calculate_grade(s.marks)
        })

    return render(request, "reports/student_report.html", {
        "student": student,
        "total": result["total"],
        "average": result["average"],
        "scores": enriched_scores
    })
    
# CLASS RANKING VIEW
def class_result_view(request, class_id):
    ranking = class_ranking(class_id)

    return render(request, "reports/class_report.html", {
        "ranking": ranking
    })